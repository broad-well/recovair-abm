...
# VALUES (4, 'WN881', 'N941WN', 'DAL', 'MDW', NULL, "2024-03-01 16:45:00", "2024-03-01 19:25:00", 'few');
    ...
    def write_throughput_disruptions(self, sim_date: str, dep_df: pd.DataFrame, arr_df: pd.DataFrame):
        ...
        def generate_plausible_times(utc_hour: int):
            sim_datetime = datetime.strptime(sim_date, '%Y-%m-%d')
            hour_datetime = sim_datetime.replace(hour=utc_hour)
            return [hour_datetime - timedelta(days=1), hour_datetime, hour_datetime + timedelta(days=1)]
        format_time = lambda dt: dt.strftime('%Y-%m-%d %H:%M:%S')

        generator = (
            (row["ORIGIN"], format_time(plausible_time), format_time(plausible_time + timedelta(hours=1)), row["THROUGHPUT"], "dep", "autogenerated", self.sid)
            for _, row in dep_df.iterrows() for plausible_time in generate_plausible_times(row["ActualDepHourUTC"])
        )
        self.conn.executemany("INSERT INTO disruptions(airport, start, end, hourly_rate, type, reason, sid) VALUES(?,?,?,?,?,?,?)", generator)
        generator = (
            (row["DEST"], format_time(plausible_time), format_time(plausible_time + timedelta(hours=1)), row["THROUGHPUT"], "gdp", "autogenerated", self.sid)
            for _, row in arr_df.iterrows() for plausible_time in generate_plausible_times(row["ActualArrHourUTC"])
        )
        self.conn.executemany("INSERT INTO disruptions(airport, start, end, hourly_rate, type, reason, sid) VALUES(?,?,?,?,?,?,?)", generator)
...
def find_abnormal_throughputs(month_df: pd.DataFrame, date: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Find all airport departure and arrival throughputs on the given date that are abnormally low.
    
    Abnormally low is defined as below the 20th percentile of all departure and arrival throughputs
    at the airport across all days of the month given.
    
    Returns: A tuple of two DataFrames.
    The first DataFrame lists abnormal departure throughputs and contains columns `ORIGIN`, `ActualDepHourUTC`, and `THROUGHPUT`.
    The second DataFrame lists abnormal arrival throughputs and contains columns `DEST`, `ActualDepHourUTC`, and `THROUGHPUT`."""
    # Goal: Compare the hourly departure and arrival count for WN at each airport to their distribution across the entire month
    d_ahi = month_df.groupby(['ORIGIN', 'ActualDepHourUTC', 'FL_DATE'])['OP_CARRIER_FL_NUM']\
        .count().groupby(['ORIGIN', 'ActualDepHourUTC']).quantile(0.2)
    r_ahi = month_df.groupby(['DEST', 'ActualArrHourUTC', 'FL_DATE'])['OP_CARRIER_FL_NUM']\
        .count().groupby(['DEST', 'ActualArrHourUTC']).quantile(0.2)

    day_d_ah = month_df[month_df['FL_DATE'] == date].groupby(['ORIGIN', 'ActualDepHourUTC'])['OP_CARRIER_FL_NUM'].count()
    df_d_abnormal = day_d_ah - d_ahi < 0
    
    day_r_ah = month_df[month_df['FL_DATE'] == date].groupby(['DEST', 'ActualArrHourUTC'])['OP_CARRIER_FL_NUM'].count()
    df_r_abnormal = day_r_ah - r_ahi < 0
    return tuple(series.reset_index().rename(columns={'OP_CARRIER_FL_NUM': 'THROUGHPUT'})
                 for series in (day_d_ah[df_d_abnormal], day_r_ah[df_r_abnormal]))
...
def seed(date: str, airport_capacity_source_start: str, airport_capacity_source_end: str, scenario_name: str, scenario_id: str, start_time: str, end_time: str, days: float, crew_mult=2, flight_source='T_ONTIME_REPORTING.csv'):
    ...
    writer.write_synthesized_itineraries(
        ...
        'WN', days=days)
    abnormal_throughputs = find_abnormal_throughputs(month_df, date)
